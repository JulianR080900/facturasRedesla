...
class CuestionariosModel extends Model{

    public function getAll($tabla,$condiciones){
        $db = \Config\Database::connect('cuestionarios');
    ...
    public function getColumnsAll($columnas,$tabla,$condiciones){
        $db = \Config\Database::connect('cuestionarios');
    ...
    public function get_max_connections() {
        $db = \Config\Database::connect('cuestionarios');
        $query = $db->query('SHOW STATUS WHERE `variable_name` = "Threads_connected"');
        $result = $query->getRow();
        $num_connections = $result->Value;
