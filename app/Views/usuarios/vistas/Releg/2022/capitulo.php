...
<body>
    <div class="container">
    <form method='post' action='../../updateCapituloReleg' class='needs-validation' novalidate> <!--addCapitulo -->
        <section>
        ...
        <section id='categorias'>
            <div>
                ...
                <input type="text" name="enfoque" id="" class="form-control" value="<?= $info['enfoque'] ?>" readonly>
            </div>
            <hr>
            <div id="hoja">
                <h3>
            ...
            foreach ($categorias as $key=>$c) {
                $autofill = $info['categoria_'.$key+1];
                $explode_fill = explode(';',$autofill);
                $explode_codigos = explode('~',$explode_fill[1]);
                $explode_codigos = array_chunk($explode_codigos,3);
                $explode_observaciones = explode(';',$info['observaciones']);
                $observacion = $explode_observaciones[$key];
            ...
            ?>
                <div id="divCat<?= $c['id'] ?>">
                    ...
                    if(!empty($observacion)){
                        ?>
                        <h3>Observaciones:</h3>
                        <p style="color:red"><?= $observacion ?></p>
                        <?php
                    }
                    ?>
                    <?php
                    ...
                    ?>
                        <?php
                        if(strpos($info['codigos_validos'], $explode_codigos[$i-1][2]) !== false){
                            ?>
                            <div class='row' id='categoria_<?= $c['id'] ?>_<?= $i ?>'>
                                <div class='col-md-3'>
                                    <input type="text" class="form-control" name='categorias[<?= $c['id'] ?>][<?= $i ?>][id_entrevista]' value="<?= $explode_codigos[$i-1][0] ?>" readonly>
                                </div>
                                <div class='col-md-9'>
                                    <input type="text" name='' id="" class="form-control" value="<?= $explode_codigos[$i-1][1] ?>" disabled>
                                    <input type="text" name='categorias[<?= $c['id'] ?>][<?= $i ?>][codigo_en_vivo]' id="" class="form-control" value="<?= $explode_codigos[$i-1][2] ?>" hidden>
                                </div>
                            </div>
                            <?php
                        }else{
                    ...
                    <?php
                    if(strpos($info['analisis_validos'], $c['nombre']) !== false){
                        ?>
                        <textarea name='categorias[<?= $c['id'] ?>][analisis]' class='form-control' placeholder="Redactar el análisis de la categoría: <?= $c['nombre'] ?>" required minlength='300' maxlength='350' readonly><?= $explode_fill[2] ?></textarea>
                        <?php
                    }else{
                        ?>
                        <h3>Observaciones:</h3>
            ...
            ?>
            <div class="row">
            <input type="text" name="id_capitulo" id="" value="<?= $info['id'] ?>" hidden>
            <button type="submit" class="btn btn-block btn-success">Actualizar información</button>
            </div>
            </div>
    ...
    <footer>
        ...
        <p class="text-center">jaramosp@e.redesla.la</p>
    </footer>
    </div>
